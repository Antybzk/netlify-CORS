...
export default async (request: Request, context: Context) => {
  ...
  if (path.startsWith('/proxy/')) {
    try {
      ...
      // --- ДОБАВЬТЕ ЭТОТ БЛОК ДЛЯ GigaChat (ОСНОВНОЙ ПРОКСИ) ---
      if (targetBaseUrl === 'https://gigachat.devices.sberbank.ru') {
        const apiKey = Deno.env.get("GIGACHAT_API_KEY");
        if (apiKey) {
          proxyRequest.headers.set('Authorization', `Basic ${apiKey}`);
          console.log('✅ Добавлена авторизация для GigaChat (основной прокси)');
        } else {
          console.error('❌ Переменная GIGACHAT_API_KEY не найдена');
          return new Response('Ошибка: API-ключ GigaChat не настроен', {
            status: 500,
  ...
  if (targetBaseUrl && matchedPrefix) {
    ...
    try {
      ...
      // --- ДОБАВЬТЕ ЭТОТ БЛОК ДЛЯ GigaChat (УНИВЕРСАЛЬНЫЙ ПРОКСИ) ---
      if (targetUrl.hostname === 'gigachat.devices.sberbank.ru') {
        const apiKey = Deno.env.get("GIGACHAT_API_KEY");
        if (apiKey) {
          proxyRequest.headers.set('Authorization', `Basic ${apiKey}`);
          console.log('✅ Добавлена авторизация для GigaChat (универсальный прокси)');
        } else {
          console.error('❌ Переменная GIGACHAT_API_KEY не найдена');
          return new Response('Ошибка: API-ключ GigaChat не настроен', {
            status: 500,
